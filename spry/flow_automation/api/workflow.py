...
def save_workflow(data):
    ...
    if frappe.db.exists("Workflow", {"workflow_name": workflow_name}):
        ...
        workflow = frappe.get_doc("Workflow", {"workflow_name": workflow_name})
        
        # Check permission
        if not frappe.has_permission("Workflow", "write", workflow):
...
def delete_workflow(name):
    """Delete workflow"""
    if not frappe.has_permission("Workflow", "delete"):
